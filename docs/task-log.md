...
- 2026-09-25: raised with Migert at GATE-0, not changed: a **partial** run on day k >= 1 leaves an unread video out of the snapshot; if it entered on day k and is read on day k+1 it gets `entry_certain = true` though its entry day is unknown. Same family: `close_exits()` (T-07) after a partial run would close videos that were only unread.
